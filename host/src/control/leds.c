...
void leds_init(leds_t* state) {
    for (int led = 0; led < NUM_LEDS; led++) { state->led_state[led] = LED_STATE_OFF; }
}

void leds_on(leds_t* state, led_type_t led) { state->led_state[led] = LED_STATE_ON; }

void leds_off(leds_t* state, led_type_t led) { state->led_state[led] = LED_STATE_OFF; }
