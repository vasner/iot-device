 ...
 */
uint8_t at32_button_state(void) { return gpio_input_data_bit_read(USER_BUTTON_PORT, USER_BUTTON_PIN); }
...
button_type at32_button_press() {
    ...
    if ((pressed == 1) && (at32_button_state() != RESET)) {
        ...
        pressed = 0;
        delay_ms(10);
        if (at32_button_state() != RESET) return USER_BUTTON;
...
void at32_led_on(led_type led) {
    if (led > (LED_NUM - 1)) return;
    if (led_gpio_pin[led]) led_gpio_port[led]->clr = led_gpio_pin[led];
}
...
void at32_led_off(led_type led) {
    if (led > (LED_NUM - 1)) return;
    if (led_gpio_pin[led]) led_gpio_port[led]->scr = led_gpio_pin[led];
}
...
void at32_led_toggle(led_type led) {
    if (led > (LED_NUM - 1)) return;
    if (led_gpio_pin[led]) led_gpio_port[led]->odt ^= led_gpio_pin[led];
}
